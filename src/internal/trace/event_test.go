...
func TestMakeEvent(t *testing.T) {
	// TODO: consistent error messages, check rest of the package
	// TODO: simplify the tests, maybe try using a dedicated event type again
		...
		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				ev, err := MakeEvent(EventConfig[Metric]{
					Kind:    EventMetric,
					Time:    42, // TODO use different time for each test
	...
	t.Run("GoroutineTransition", func(t *testing.T) {
		...
		tests := []struct {
			...
			// TODO: Use coverage testsing to check if we need all these GoStatus/GoStatusStack cases
			{
